    ...
    where TUpdateDto : class
{
    ...
    {
        var validatedModel = await ValidateUpdateModelAsync(entity, model);
        MapToEntity(validatedModel, entity);
        await ValidateUpdatedEntityAsync(entity);
    }

    // islevi: Entity update davranisinin urettigi kanonik alanlara dayali domain kurallarini concrete servise acar.
    protected virtual Task<TEntity> ValidateUpdatedEntityAsync(TEntity entity)
    {
