    ...
    where TCreateDto : class
{
    ...
    protected virtual async Task<TEntity> CreateEntityAsync(TCreateModel model)
    {
        ...
        var entity = BuildEntity(validatedModel);
        return await ValidateCreatedEntityAsync(entity);
    }
    ...
    protected virtual async Task<List<TEntity>> CreateEntitiesAsync(List<TCreateModel> models)
    {
        ...
        var entities = validatedModels.Select(BuildEntity).ToList();
        return await ValidateCreatedEntitiesAsync(entities);
    }

    // islevi: Entity kurulusunda uretilen kanonik alanlara dayali domain kurallarini concrete servise acar.
    protected virtual Task<TEntity> ValidateCreatedEntityAsync(TEntity entity)
    {
        return Task.FromResult(entity);
    }

    // islevi: Toplu entity kurulusundan sonra kanonik anahtarlarin tek sorguyla dogrulanabilmesini saglar.
    protected virtual Task<List<TEntity>> ValidateCreatedEntitiesAsync(List<TEntity> entities)
    {
        return Task.FromResult(entities);
    }

    // Dogrulanmis create modelinden aggregate kurallarini koruyan yeni entity'yi uretir.
    protected abstract TEntity BuildEntity(TCreateModel model);
