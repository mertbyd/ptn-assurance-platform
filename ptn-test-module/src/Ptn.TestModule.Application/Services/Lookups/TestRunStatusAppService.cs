...
// islevi: Kosum durumu lookup'inin okuma use-case'lerini Foundation lookup tabani uzerinden sunar.
// sistemdeki gorevi: Okuma akisi tabandan gelir; bu tip yalniz okuma policy'sini ve Mapperly eslemesini sahiplenir.
[RemoteService(IsEnabled = false)]
public class TestRunStatusAppService : LookupAppService<
    TestRunStatus,
    Guid,
    TestRunStatusDto,
    LookupCreateDto,
    LookupUpdateDto,
    TestRunStatusManager,
    ITestRunStatusRepository>, ITestRunStatusAppService
{
    ...
    protected override TestRunStatusDto MapToDto(TestRunStatus entity)
    {
        return Mapper.Map(entity);
    }

    // Lookup sayfasini tek Mapperly collection eslemesiyle DTO listesine cevirir.
    protected override List<TestRunStatusDto> MapToDto(IReadOnlyList<TestRunStatus> entities)
    {
