...
// islevi: Test hukmu lookup'inin okuma use-case'lerini Foundation lookup tabani uzerinden sunar.
// sistemdeki gorevi: Okuma akisi tabandan gelir; bu tip yalniz okuma policy'sini ve Mapperly eslemesini sahiplenir.
[RemoteService(IsEnabled = false)]
public class TestOutcomeStatusAppService : LookupAppService<
    TestOutcomeStatus,
    Guid,
    TestOutcomeStatusDto,
    LookupCreateDto,
    LookupUpdateDto,
    TestOutcomeStatusManager,
    ITestOutcomeStatusRepository>, ITestOutcomeStatusAppService
{
    ...
    // Lookup satirini build politikasiyla birlikte public okuma DTO'suna cevirir.
    protected override TestOutcomeStatusDto MapToDto(TestOutcomeStatus entity)
    {
        return Mapper.Map(entity);
    }

    // Lookup sayfasini tek Mapperly collection eslemesiyle DTO listesine cevirir.
    protected override List<TestOutcomeStatusDto> MapToDto(IReadOnlyList<TestOutcomeStatus> entities)
    {
