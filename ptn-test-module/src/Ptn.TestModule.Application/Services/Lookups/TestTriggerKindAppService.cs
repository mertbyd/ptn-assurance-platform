...
// islevi: Tetikleme turu lookup'inin okuma use-case'lerini Foundation lookup tabani uzerinden sunar.
// sistemdeki gorevi: Okuma akisi tabandan gelir; bu tip yalniz okuma policy'sini ve Mapperly eslemesini sahiplenir.
[RemoteService(IsEnabled = false)]
public class TestTriggerKindAppService : LookupAppService<
    TestTriggerKind,
    Guid,
    TestTriggerKindDto,
    LookupCreateDto,
    LookupUpdateDto,
    TestTriggerKindManager,
    ITestTriggerKindRepository>, ITestTriggerKindAppService
{
    ...
    protected override TestTriggerKindDto MapToDto(TestTriggerKind entity)
    {
        return Mapper.Map(entity);
    }

    // Lookup sayfasini tek Mapperly collection eslemesiyle DTO listesine cevirir.
    protected override List<TestTriggerKindDto> MapToDto(IReadOnlyList<TestTriggerKind> entities)
    {
